...
def discount_item_filter(request):
  if request.method == 'GET':
    from_date = request.GET.get('startD')
    to_date = request.GET.get('endD')
    date_obj1 = datetime.strptime(from_date, '%a %b %d %Y')
    ...
    staff =  staff_details.objects.get(id=sid)
    cid= staff.company.id
    sales_data=SalesInvoiceItem.objects.filter(company=cid)
    distinct_items_with_discount = SalesInvoiceItem.objects.filter(company=cid,salesinvoice__date__range=(startD, toD)).values('item__item_name').annotate(total_discount_amount=Sum('discount'),total_quantity=Sum('quantity'),total_sales_amount=Sum('totalamount'))
    results = []
    if distinct_items_with_discount.exists():
      for i in distinct_items_with_discount:
          results.append({
              'item_name': i.item__item_name,
              'Quantity': i.total_quantity,
              'Price': i.total_sales_amount,
              'Discount': i.total_discount_amount 
          })
    else:
      results = [{'item_name': '', 'Quantity': 0, 'Price': 0, 'Discount': 0}]
    print(results)
    return JsonResponse({'parties': results})
  else:
    return HttpResponse(status=400)
...
def Item_Wise_Discount_Report1(request):
    id=request.session.get('staff_id')
    staff=staff_details.objects.get(id=id)
    cmp = company.objects.get(id=staff.company.id) 
    allmodules= modules_list.objects.get(company=cmp,status='New')
    ...
    return render(request,'company/trial.html',content)

def send_Item_Wise_Discount_report_via_mail(request):
  if request.method == 'POST':
    from_date_str=request.POST['fdate']
    To_date_str=request.POST['tdate']
    search=request.POST['search']
    ...
    if from_date_str and To_date_str:
      ...
      template_path = 'company/trial.html'
      template = get_template(template_path)
      ...
      pdf = pisa.pisaDocument(BytesIO(html.encode("ISO-8859-1")), result)
      pdf = result.getvalue()
      filename = f'Item Wise Discount Report.pdf'
      email = EmailMessage(mess,from_email=settings.EMAIL_HOST_USER,to=emails)
      email.attach(filename, pdf, "application/pdf")
      email.send(fail_silently=False)
      messages.info(request,'Item Wise Discount report shared via mail')
      return redirect('Item_Wise_Discount_Report')
    #if search input -------------------------
    if search:
      print(search)
      if PurchaseBill.objects.filter(billdate__startswith=search) or  purchasedebit.objects.filter(billdate__startswith=search):
